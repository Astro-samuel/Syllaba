...
/**
 * Builds a fresh, fully-configured McpServer with all 5 read-only tools
 * registered. A new instance per HTTP request in stateless mode (see
 * index.ts) — the SDK's McpServer isn't meant to be shared across
 * concurrent unrelated requests, and these tools have no per-connection
 * state to preserve anyway (every call re-reads the data file from disk).
 */
export function createSyllabaMcpServer(): McpServer {
